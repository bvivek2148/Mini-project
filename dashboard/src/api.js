...
export async function simulateRansomware(target_dir = null) {
    const body = target_dir ? JSON.stringify({ target_dir }) : undefined;
    const res = await fetch(`${BASE}/agent/simulate`, {
        method: 'POST',
        headers: body ? { 'Content-Type': 'application/json' } : undefined,
        body
    })
    if (!res.ok) throw new Error(`Failed to trigger simulation: ${res.status}`)
    return res.json()
}

export async function undoSimulation(target_dir = null) {
    const body = target_dir ? JSON.stringify({ target_dir }) : undefined;
    const res = await fetch(`${BASE}/agent/simulate/undo`, {
        method: 'POST',
        headers: body ? { 'Content-Type': 'application/json' } : undefined,
        body
    })
    if (!res.ok) throw new Error(`Failed to trigger simulation undo: ${res.status}`)
    return res.json()
}
