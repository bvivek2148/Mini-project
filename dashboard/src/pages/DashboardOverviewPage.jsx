            ...
            <div className="flex gap-2 relative">
              ...
              <div className="relative" ref={notifRef}>
                <button
                  ...
                  className="relative flex size-10 items-center justify-center rounded-lg bg-[#233648] text-white hover:bg-[#334b63] transition-colors"
                ...
                >
                  ...
                  {recentAlerts.length > 0 && (
                    <span className="absolute top-1.5 right-1.5 flex size-3">
                      ...
                      <span className="relative inline-flex rounded-full size-3 bg-red-500 border-2 border-[#233648]" />
                    </span>
                  )}
                </button>

                {showNotifications && (
                  <div className="absolute right-0 mt-2 w-[420px] bg-[#111a24] border border-[#2a3c4f] rounded-2xl shadow-2xl shadow-black/40 z-50 overflow-hidden" style={{ animation: 'fadeInScale 0.2s ease-out' }}>
                    {/* Header */}
                    <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#2a3c4f] bg-gradient-to-r from-[#1a2a3a] to-[#1e2e3e]">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-widest text-[#92adc9] bg-[#233648] px-2.5 py-1 rounded-full">{recentAlerts.length} new</span>
                    </div>
                    ...
                    <div className="max-h-[480px] overflow-y-auto">
                      {recentAlerts.length === 0 ? (
                        <div className="p-10 text-center">
                          <span className="material-symbols-outlined text-5xl text-[#2a3c4f] mb-3 block">notifications_off</span>
                          <p className="text-[#556980] text-sm font-medium">No recent alerts</p>
                          <p className="text-[#3a5068] text-xs mt-1">Your system is quiet — for now.</p>
                        </div>
                      ) : (
                        <div className="flex flex-col">
                          {recentAlerts.map((alert, i) => {
                            const isRansomware = alert.alert_type === 'ransomware_suspected'
                            const borderColor = isRansomware ? 'border-l-red-500' : 'border-l-amber-500'
                            const iconBg = isRansomware ? 'bg-red-500/15' : 'bg-amber-500/15'
                            const iconColor = isRansomware ? 'text-red-400' : 'text-amber-400'
                            return (
                              <div key={i} className={`border-l-[3px] ${borderColor} border-b border-b-[#1e2e3e] hover:bg-[#1a2a3a] transition-all duration-200 cursor-pointer`}>
                                <div className="px-4 py-3.5">
                                  {/* Title Row */}
                                  <div className="flex items-start gap-3">
                                    <div className={`flex-shrink-0 size-8 rounded-lg ${iconBg} flex items-center justify-center mt-0.5`}>
                                      <span className={`material-symbols-outlined text-[16px] ${iconColor}`}>
                                        {isRansomware ? 'shield' : 'pest_control'}
                                      </span>
                                    </div>
                                    <div className="flex-1 min-w-0">
                                      <div className="flex items-center justify-between gap-2">
                                        <span className="text-white text-[13px] font-semibold leading-tight">
                                          {isRansomware ? '🚨 Ransomware Encryption Detected' : '⚠️ Honeytoken File Accessed'}
                                        </span>
                                        <span className="flex-shrink-0 text-[#556980] text-[10px] font-mono tabular-nums">
                                          {alert.timestamp ? new Date(alert.timestamp * 1000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }) : 'Just now'}
                                        </span>
                                      </div>
                                      ...
                                      <div className="mt-2 space-y-1">
                                        <div className="flex items-center gap-2 text-[11px]">
                                          <span className="text-[#556980] w-10 flex-shrink-0">Host</span>
                                          <span className="text-[#92adc9] font-mono">{alert.host || '—'}</span>
                                        </div>
                                        {alert.path && (
                                          <div className="flex items-center gap-2 text-[11px]">
                                            <span className="text-[#556980] w-10 flex-shrink-0">File</span>
                                            <span className="text-[#92adc9] font-mono truncate" title={alert.path}>{alert.path.split('\\').pop() || alert.path.split('/').pop()}</span>
                                          </div>
                                        )}
                                        {isRansomware && alert.process_name && (
                                          <div className="flex items-center gap-2 text-[11px]">
                                            <span className="text-[#556980] w-10 flex-shrink-0">Proc</span>
                                            <span className="text-[#92adc9] font-mono">{alert.process_name}</span>
                                            {alert.pid && <span className="text-[#3a5068] font-mono">(PID: {alert.pid})</span>}
                                          </div>
                                        )}
                                        {isRansomware && alert.remote_ips && alert.remote_ips.length > 0 && (
                                          <div className="flex items-center gap-2 text-[11px]">
                                            <span className="text-[#556980] w-10 flex-shrink-0">C&C</span>
                                            <span className="text-red-400 font-mono">{alert.remote_ips.join(', ')}</span>
                                          </div>
                                        )}
                                      </div>
                                      ...
                                      {isRansomware && (
                                        <div className="flex flex-wrap gap-1.5 mt-2.5">
                                          {alert.process_killed !== undefined && (
                                            <span className={`inline-flex items-center gap-1 text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${alert.process_killed ? 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
                                              <span className="material-symbols-outlined text-[10px]">{alert.process_killed ? 'check_circle' : 'cancel'}</span>
                                              {alert.process_killed ? 'Process Killed' : 'Kill Failed'}
                                            </span>
                                          )}
                                          {alert.folder_locked !== undefined && (
                                            <span className={`inline-flex items-center gap-1 text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${alert.folder_locked ? 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
                                              <span className="material-symbols-outlined text-[10px]">{alert.folder_locked ? 'lock' : 'lock_open'}</span>
                                              {alert.folder_locked ? 'Folder Locked' : 'Lock Failed'}
                                            </span>
                                          )}
                                        </div>
                                      )}
                    ...
                    {/* Footer */}
                    <div className="border-t border-[#2a3c4f] bg-[#0e1820]">
                      <Link
                        to="/alerts"
                        onClick={() => setShowNotifications(false)}
                        className="flex items-center justify-center gap-2 px-4 py-3 text-primary text-xs font-bold uppercase tracking-wider hover:bg-[#1a2a3a] transition-colors"
                      >
              ...
              >
                <span className={`material-symbols-outlined text-[20px] ${agentToggling ? 'animate-spin' : ''}`}>restart_alt</span>
              </button>
              <button onClick={() => window.location.reload()} className="flex size-10 items-center justify-center rounded-lg bg-[#233648] text-white hover:bg-[#334b63] transition-colors" title="Refresh Dashboard">
                <span className="material-symbols-outlined text-[20px] active:rotate-180 transition-transform duration-300">refresh</span>
              </button>
