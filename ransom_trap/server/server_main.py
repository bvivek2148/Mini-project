...
async def create_alert(request: Request) -> Dict[str, Any]:
    ...
    import threading
    threading.Thread(target=_dispatch_notifications, args=(data,), daemon=True).start()
    return {"status": "ok"}
...
async def patch_config(request: Request) -> Dict[str, Any]:
    ...
    if "monitored_paths" in patch:
        ...
        agent["honeytoken_paths"] = patch["monitored_paths"]
...
async def agent_simulate_undo(req: SimulateRequest | None = None) -> Dict[str, Any]:
    """Execute the simulated ransomware script with the undo argument to restore original test files."""
    script_path = BASE_DIR.parent / "test_files" / "simulate_ransomware.py"
    if not script_path.exists():
        return JSONResponse({"error": "Simulation script not found"}, status_code=404)
    ...
    try:
        subprocess.Popen(
            [sys.executable, str(script_path), "--undo", target_dir],
            cwd=str(BASE_DIR.parent / "test_files")
        )
        return {"status": "started", "message": f"Simulation undo triggered on {target_dir}"}
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500)
